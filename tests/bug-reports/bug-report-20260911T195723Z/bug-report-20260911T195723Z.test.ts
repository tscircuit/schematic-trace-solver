...
test("bug-report-20260911T195723Z", () => {
  ...
  expect(v3v3Connector?.tracePath[0]).toEqual({ x: -6.5, y: -19 })
  ...
  expect(v3v3Connector?.tracePath.at(-1)).toEqual(v3v3Label!.anchorPoint)

  const collisionSafeV3v3Label =
    solver.netLabelToTraceSolver!.outputNetLabelPlacements.find(
      (label) =>
        label.netId === "V3V3" && label.pinIds.includes("schematic_port_92"),
    )
  expect(collisionSafeV3v3Label?.anchorPoint.x).toBeCloseTo(-2.15)
  expect(collisionSafeV3v3Label?.anchorPoint.y).toBeCloseTo(-7.999)
