...
test("mergeSameNetTraceSegments avoids merging through different-net traces", () => {
  const [a, b] = mergeSameNetTraceSegments([
    makeTrace("a", "net1", [
      { x: 0, y: 0 },
      { x: 0, y: 1 },
  ...
  expect(a!.tracePath[1]!.y).toBe(1)
  expect(a!.tracePath[2]!.y).toBe(1)
  expect(b!.tracePath[1]!.y).toBe(1.1)
  expect(b!.tracePath[2]!.y).toBe(1.1)
})
