...
describe("scan all examples for duplicate trace segments", () => {
  for (const name of exampleFiles) {
    test(`${name}`, () => {
      ...
      const lastStepLines = (graphics.lines ?? []).filter(
        (l: any) => l.step === lastStep,
      )
      ...
      for (const [key, { count, keys }] of segmentMap) {
        if (count > 1) {
          dups++
          console.log(
            `  dup(${count}x) ${key} in ${[...new Set(keys)].join(", ")}`,
          )
        }
      }
      ...
      if (dups > 0) {
        console.log(
          `${name}: ${dups}/${segmentMap.size} duplicate segments (${lastStepLines.length} lines)`,
        )
      }
