...
test("example32 - no duplicate trace segments at last step", () => {
  ...
  for (const [key, {count, traceIdx}] of segmentMap) {
    if (count > 1) {
      dupCount++
      console.log(`  Duplicate segment (${count}x): ${key} in traces [${[...new Set(traceIdx)].join(',')}]`)
    }
  }
  console.log(`Total unique segments: ${segmentMap.size}, duplicate segments: ${dupCount}`)
