...
async function main() {
  ...
  try {
    ...
    const textarea = iframe.locator(
      'textarea[placeholder="Paste your InputProblem here..."]',
    )
    ...
    console.log(
      "Showing solved schematic pipeline (no traces between C1 and C2, only net labels)...",
    )
    ...
    const videoFile = files.find(
      (f) => f.endsWith(".webm") && f !== "repro79_demo.webm",
    )
    if (videoFile) {
