...
// It emits the raw two-point path, producing a 4.71-long wire with 0.01 of
// vertical drift: a visibly slanted schematic trace.
test("recovered net-label traces come out diagonal", () => {
  ...
  // Current behaviour: the recovered trace carries a slanted segment.
  expect(diagonalSegments.length).toBeGreaterThan(0)

  // The recovery itself does happen, the path is just not orthogonal.
  expect(
    traces.some((trace) =>
