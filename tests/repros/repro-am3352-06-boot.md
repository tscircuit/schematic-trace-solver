# AM3352 full 06-Boot sheet

Reproduces the complete **06-Boot — SYSBOOT selectors** sheet from the supplied
`am3352-dev-board-4layer-dogbone.json`, including all 33 components, 96 ports,
and 34 named nets.
...
schematic component/port IDs, pin coordinates, facing directions, and named nets
are preserved. Net membership follows source-port connectivity keys. All 96
touching source traces are port-to-net connections, so direct connections are
empty. Existing routed traces and labels are reference output only; they are
not fed back into the solver.

Component obstacles and 18 component-owned text obstacles use core's
...
```sh
bun test tests/repros/repro-am3352-06-boot.test.ts
bun run debug:pipeline tests/repros/assets/repro-am3352-06-boot.input.json --svg
```

For interactive inspection, run `bun start` and open
`SchematicTracePipelineSolver/repro-am3352-06-boot`.
