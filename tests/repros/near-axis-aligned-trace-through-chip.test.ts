...
// matches neither fast path in segmentIntersectsRect and is reported as
// colliding with nothing. The trace then runs straight through
...
test("a nearly-vertical segment misses a rect it passes through", () => {
  ...
  // Current behaviour: the skew is ~6e-5, far below the routing grid but far
  // above the 1e-9 epsilon, so neither axis-aligned fast path matches and the
  // segment is reported as hitting nothing.
  expect(segmentIntersectsRect(skewedStart, skewedEnd, chipRect)).toBe(false)
...
test("recovered traces run through a chip they do not terminate on", () => {
  ...
  for (const trace of traces) {
    for (let index = 0; index + 1 < trace.tracePath.length; index++) {
      ...
      if (crossesChipInterior) tracesThroughChip++
    }
  }

  // Current behaviour: the trace cuts straight through schematic_component_8.
  expect(tracesThroughChip).toBeGreaterThan(0)
})
