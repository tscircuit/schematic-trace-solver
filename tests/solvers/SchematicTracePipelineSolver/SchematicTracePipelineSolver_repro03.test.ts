...
test("SchematicTracePipelineSolver_repro03 - infinite loop fixed", () => {
  console.log("Starting SchematicTracePipelineSolver test with debugging...")
  ...
  // Patch the _step method to add step counting and timeout  
  const originalStep = solver._step.bind(solver)
  solver._step = function() {
    stepCount++
    
    if (stepCount > maxSteps) {
      throw new Error(`Solver exceeded maximum steps (${maxSteps}). Infinite loop detected.`)
    }
    
    // Log periodically 
    if (stepCount % 100 === 0 || stepCount <= 5) {
      console.log(`Step ${stepCount}: phase=${solver.getCurrentPhase()}, solved=${solver.solved}`)
      
      if (solver.activeSubSolver) {
        const subsolverName = solver.activeSubSolver.constructor.name
        const iterations = solver.activeSubSolver.iterations || 0
        console.log(`  Active: ${subsolverName} (iteration ${iterations})`)
        
        if (subsolverName === 'GuidelinesSolver') {
          const guidelinesSolver = solver.activeSubSolver
          const guidelines = guidelinesSolver.guidelines?.length || 0
          const chips = guidelinesSolver.inputProblem?.chips?.length || 0
          const expected = chips * (chips - 1)
          console.log(`  Guidelines: ${guidelines}, Expected pairs: ${expected}`)
        } else if (subsolverName === 'SchematicTraceLinesSolver') {
          const traceSolver = solver.activeSubSolver
          console.log(`  Queue length: ${traceSolver.queuedConnectionPairs?.length || 0}`)
          console.log(`  Solved traces: ${traceSolver.solvedTracePaths?.length || 0}`)
          console.log(`  Failed traces: ${traceSolver.failedConnectionPairs?.length || 0}`)
          console.log(`  Has active sub: ${!!traceSolver.activeSubSolver}`)
          
          if (traceSolver.activeSubSolver) {
            const singleLineSolver = traceSolver.activeSubSolver
            console.log(`  Single line solver: solved=${singleLineSolver.solved}, failed=${singleLineSolver.failed}`)
            console.log(`  Candidate paths remaining: ${singleLineSolver.queuedCandidatePaths?.length || 0}`)
          }
        }
      }
    }
  ...
  try {
    ...
    console.log(`Test completed successfully after ${stepCount} steps`)
    
    // Basic validation that solver completed
    expect(solver.solved).toBe(true)
    expect(result).toBeDefined()
    
  } catch (error) {
    console.error(`Test failed after ${stepCount} steps:`, error.message)
    throw error
  }
})