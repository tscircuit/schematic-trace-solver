import type { InputProblem } from "lib/types/InputProblem"
import { test, expect } from "bun:test"
import { SchematicTracePipelineSolver } from "lib/index"

const inputProblem: InputProblem = {
  chips: [
    {
      chipId: "schematic_component_0",
      center: {
        x: 0,
        y: 0,
      },
      width: 2.3,
      height: 1.4,
      pins: [
        {
          pinId: "J1.1",
          x: 1.15,
          y: 0.5,
        },
        {
          pinId: "J1.2",
          x: 1.15,
          y: 0.30000000000000004,
        },
        {
          pinId: "J1.3",
          x: 1.15,
          y: 0.10000000000000009,
        },
        {
          pinId: "J1.4",
          x: 1.15,
          y: -0.09999999999999998,
        },
        {
          pinId: "J1.5",
          x: 1.15,
          y: -0.3,
        },
        {
          pinId: "J1.6",
          x: 1.15,
          y: -0.5,
        },
      ],
    },
    {
      chipId: "schematic_component_1",
      center: {
        x: 1.56,
        y: 4.308333333333334,
      },
      width: 1,
      height: 0.45,
      pins: [
        {
          pinId: "SW1.1",
          x: 1.06,
          y: 4.258333333333334,
        },
        {
          pinId: "SW1.2",
          x: 2.06,
          y: 4.258333333333334,
        },
      ],
    },
    {
      chipId: "schematic_component_2",
      center: {
        x: 0.3599999999999999,
        y: 4.308333333333334,
      },
      width: 1.04,
      height: 0.54,
      pins: [
        {
          pinId: "D1.1",
          x: -0.16000000000000014,
          y: 4.308333333333334,
        },
        {
          pinId: "D1.2",
          x: 0.8799999999999999,
          y: 4.308333333333334,
        },
      ],
    },
    {
      chipId: "schematic_component_3",
      center: {
        x: 5.06,
        y: 0.3583333333333334,
      },
      width: 1,
      height: 0.45,
      pins: [
        {
          pinId: "SW2.1",
          x: 4.56,
          y: 0.30833333333333357,
        },
        {
          pinId: "SW2.2",
          x: 5.56,
          y: 0.30833333333333357,
        },
      ],
    },
    {
      chipId: "schematic_component_4",
      center: {
        x: 3.8599999999999994,
        y: 0.3583333333333334,
      },
      width: 1.040000000000001,
      height: 0.54,
      pins: [
        {
          pinId: "D2.1",
          x: 3.339999999999999,
          y: 0.3583333333333334,
        },
        {
          pinId: "D2.2",
          x: 4.38,
          y: 0.3583333333333334,
        },
      ],
    },
    {
      chipId: "schematic_component_5",
      center: {
        x: 3.163333333333334,
        y: 10.185,
      },
      width: 1,
      height: 0.45,
      pins: [
        {
          pinId: "SW3.1",
          x: 2.663333333333334,
          y: 10.135,
        },
        {
          pinId: "SW3.2",
          x: 3.663333333333334,
          y: 10.135,
        },
      ],
    },
    {
      chipId: "schematic_component_6",
      center: {
        x: 1.9633333333333338,
        y: 10.185,
      },
      width: 1.040000000000001,
      height: 0.54,
      pins: [
        {
          pinId: "D3.1",
          x: 1.4433333333333334,
          y: 10.185,
        },
        {
          pinId: "D3.2",
          x: 2.4833333333333343,
          y: 10.185,
        },
      ],
    },
    {
      chipId: "schematic_component_7",
      center: {
        x: 3.8100000000000005,
        y: 3.715,
      },
      width: 1,
      height: 0.45,
      pins: [
        {
          pinId: "SW4.1",
          x: 3.3100000000000005,
          y: 3.665,
        },
        {
          pinId: "SW4.2",
          x: 4.3100000000000005,
          y: 3.665,
        },
      ],
    },
    {
      chipId: "schematic_component_8",
      center: {
        x: 2.6100000000000003,
        y: 3.715,
      },
      width: 1.04,
      height: 0.54,
      pins: [
        {
          pinId: "D4.1",
          x: 2.0900000000000003,
          y: 3.715,
        },
        {
          pinId: "D4.2",
          x: 3.1300000000000003,
          y: 3.715,
        },
      ],
    },
    {
      chipId: "schematic_component_9",
      center: {
        x: 3.34,
        y: 5.783333333333333,
      },
      width: 1,
      height: 0.45,
      pins: [
        {
          pinId: "SW5.1",
          x: 2.84,
          y: 5.733333333333333,
        },
        {
          pinId: "SW5.2",
          x: 3.84,
          y: 5.733333333333333,
        },
      ],
    },
    {
      chipId: "schematic_component_10",
      center: {
        x: 2.1399999999999997,
        y: 5.783333333333333,
      },
      width: 1.040000000000001,
      height: 0.54,
      pins: [
        {
          pinId: "D5.1",
          x: 1.6199999999999992,
          y: 5.783333333333333,
        },
        {
          pinId: "D5.2",
          x: 2.66,
          y: 5.783333333333333,
        },
      ],
    },
    {
      chipId: "schematic_component_11",
      center: {
        x: 1.4249999999999998,
        y: 2.17,
      },
      width: 1,
      height: 0.45,
      pins: [
        {
          pinId: "SW6.1",
          x: 0.9249999999999998,
          y: 2.12,
        },
        {
          pinId: "SW6.2",
          x: 1.9249999999999998,
          y: 2.12,
        },
      ],
    },
    {
      chipId: "schematic_component_12",
      center: {
        x: 0.22499999999999964,
        y: 2.17,
      },
      width: 1.040000000000001,
      height: 0.54,
      pins: [
        {
          pinId: "D6.1",
          x: -0.2950000000000008,
          y: 2.17,
        },
        {
          pinId: "D6.2",
          x: 0.7450000000000001,
          y: 2.17,
        },
      ],
    },
    {
      chipId: "schematic_component_13",
      center: {
        x: 4.9799999999999995,
        y: -3.4499999999999997,
      },
      width: 1,
      height: 0.45,
      pins: [
        {
          pinId: "SW7.1",
          x: 4.4799999999999995,
          y: -3.4999999999999996,
        },
        {
          pinId: "SW7.2",
          x: 5.4799999999999995,
          y: -3.4999999999999996,
        },
      ],
    },
    {
      chipId: "schematic_component_14",
      center: {
        x: 3.7799999999999994,
        y: -3.4499999999999997,
      },
      width: 1.04,
      height: 0.54,
      pins: [
        {
          pinId: "D7.1",
          x: 3.2599999999999993,
          y: -3.4499999999999997,
        },
        {
          pinId: "D7.2",
          x: 4.299999999999999,
          y: -3.4499999999999997,
        },
      ],
    },
    {
      chipId: "schematic_component_15",
      center: {
        x: 2.944999999999998,
        y: -6.048333333333334,
      },
      width: 1,
      height: 0.45,
      pins: [
        {
          pinId: "SW8.1",
          x: 2.444999999999998,
          y: -6.098333333333334,
        },
        {
          pinId: "SW8.2",
          x: 3.444999999999998,
          y: -6.098333333333334,
        },
      ],
    },
    {
      chipId: "schematic_component_16",
      center: {
        x: 1.744999999999998,
        y: -6.048333333333334,
      },
      width: 1.04,
      height: 0.54,
      pins: [
        {
          pinId: "D8.1",
          x: 1.224999999999998,
          y: -6.048333333333334,
        },
        {
          pinId: "D8.2",
          x: 2.264999999999998,
          y: -6.048333333333334,
        },
      ],
    },
    {
      chipId: "schematic_component_17",
      center: {
        x: 4.409999999999998,
        y: -7.788333333333336,
      },
      width: 1,
      height: 0.45,
      pins: [
        {
          pinId: "SW9.1",
          x: 3.9099999999999984,
          y: -7.838333333333335,
        },
        {
          pinId: "SW9.2",
          x: 4.909999999999998,
          y: -7.838333333333335,
        },
      ],
    },
    {
      chipId: "schematic_component_18",
      center: {
        x: 3.209999999999998,
        y: -7.788333333333336,
      },
      width: 1.040000000000001,
      height: 0.54,
      pins: [
        {
          pinId: "D9.1",
          x: 2.6899999999999977,
          y: -7.788333333333336,
        },
        {
          pinId: "D9.2",
          x: 3.7299999999999986,
          y: -7.788333333333336,
        },
      ],
    },
  ],
  directConnections: [
    {
      pinIds: ["D1.2", "SW1.1"],
      netId: ".D1 > .cathode to .SW1 > .pin1",
    },
    {
      pinIds: ["D2.2", "SW2.1"],
      netId: ".D2 > .cathode to .SW2 > .pin1",
    },
    {
      pinIds: ["D3.2", "SW3.1"],
      netId: ".D3 > .cathode to .SW3 > .pin1",
    },
    {
      pinIds: ["D4.2", "SW4.1"],
      netId: ".D4 > .cathode to .SW4 > .pin1",
    },
    {
      pinIds: ["D5.2", "SW5.1"],
      netId: ".D5 > .cathode to .SW5 > .pin1",
    },
    {
      pinIds: ["D6.2", "SW6.1"],
      netId: ".D6 > .cathode to .SW6 > .pin1",
    },
    {
      pinIds: ["D7.2", "SW7.1"],
      netId: ".D7 > .cathode to .SW7 > .pin1",
    },
    {
      pinIds: ["D8.2", "SW8.1"],
      netId: ".D8 > .cathode to .SW8 > .pin1",
    },
    {
      pinIds: ["D9.2", "SW9.1"],
      netId: ".D9 > .cathode to .SW9 > .pin1",
    },
  ],
  netConnections: [
    {
      netId: "ROW0",
      pinIds: ["J1.1", "D1.1", "D2.1", "D3.1"],
    },
    {
      netId: "ROW1",
      pinIds: ["J1.2", "D4.1", "D5.1", "D6.1"],
    },
    {
      netId: "ROW2",
      pinIds: ["J1.3", "D7.1", "D8.1", "D9.1"],
    },
  ],
  availableNetLabelOrientations: {
    ROW0: ["x-", "x+"],
    ROW1: ["x-", "x+"],
    ROW2: ["x-", "x+"],
  },
  maxMspPairDistance: 5,
}

test("SchematicTracePipelineSolver_repro03 - infinite loop fixed", () => {
  console.log("Starting SchematicTracePipelineSolver test with debugging...")

  const solver = new SchematicTracePipelineSolver(inputProblem)

  let stepCount = 0
  const maxSteps = 2000

  // Patch the _step method to add step counting and timeout
  const originalStep = solver._step.bind(solver)
  solver._step = function () {
    stepCount++

    if (stepCount > maxSteps) {
      throw new Error(
        `Solver exceeded maximum steps (${maxSteps}). Infinite loop detected.`,
      )
    }

    // Log periodically
    if (stepCount % 100 === 0 || stepCount <= 5) {
      console.log(
        `Step ${stepCount}: phase=${solver.getCurrentPhase()}, solved=${solver.solved}`,
      )

      if (solver.activeSubSolver) {
        const subsolverName = solver.activeSubSolver.constructor.name
        const iterations = solver.activeSubSolver.iterations || 0
        console.log(`  Active: ${subsolverName} (iteration ${iterations})`)

        if (subsolverName === "GuidelinesSolver") {
          const guidelinesSolver = solver.activeSubSolver
          const guidelines = guidelinesSolver.guidelines?.length || 0
          const chips = guidelinesSolver.inputProblem?.chips?.length || 0
          const expected = chips * (chips - 1)
          console.log(
            `  Guidelines: ${guidelines}, Expected pairs: ${expected}`,
          )
        } else if (subsolverName === "SchematicTraceLinesSolver") {
          const traceSolver = solver.activeSubSolver
          console.log(
            `  Queue length: ${traceSolver.queuedConnectionPairs?.length || 0}`,
          )
          console.log(
            `  Solved traces: ${traceSolver.solvedTracePaths?.length || 0}`,
          )
          console.log(
            `  Failed traces: ${traceSolver.failedConnectionPairs?.length || 0}`,
          )
          console.log(`  Has active sub: ${!!traceSolver.activeSubSolver}`)

          if (traceSolver.activeSubSolver) {
            const singleLineSolver = traceSolver.activeSubSolver
            console.log(
              `  Single line solver: solved=${singleLineSolver.solved}, failed=${singleLineSolver.failed}`,
            )
            console.log(
              `  Candidate paths remaining: ${singleLineSolver.queuedCandidatePaths?.length || 0}`,
            )
          }
        }
      }
    }

    const result = originalStep.call(this)
    return result
  }

  try {
    const result = solver.solve()
    console.log(`Test completed successfully after ${stepCount} steps`)

    // Basic validation that solver completed
    expect(solver.solved).toBe(true)
    expect(result).toBeDefined()
  } catch (error) {
    console.error(`Test failed after ${stepCount} steps:`, error.message)
    throw error
  }
})
