...
export class SameNetTraceMergeSolver extends BaseSolver {
  ...
  constructor(params: { inputProblem: InputProblem; traces: any[]; netLabelPlacements?: any[] }) {
    super();
  ...
  override _step() {
    ...
    const segmentsByNet = this.traces.reduce((acc, seg) => {
    ...
    for (const netId in segmentsByNet) {
      ...
      const verticals = netSegments.filter((s: any) => Math.abs(s.x1 - s.x2) < EPSILON);
      const horizontals = netSegments.filter((s: any) => Math.abs(s.y1 - s.y2) < EPSILON);
      const others = netSegments.filter(
        (s: any) => Math.abs(s.x1 - s.x2) >= EPSILON && Math.abs(s.y1 - s.y2) >= EPSILON
      );
  ...
  private mergeOrthogonal(segments: any[], fixedAxis: 'x' | 'y', slidingAxis: 'x' | 'y') {
    const EPSILON = 0.1;
    ...
    for (const seg of segments) {
      if (seg[`${slidingAxis}1`] > seg[`${slidingAxis}2`]) {
        [seg[`${slidingAxis}1`], seg[`${slidingAxis}2`]] = [seg[`${slidingAxis}2`], seg[`${slidingAxis}1`]];
        [seg[`${fixedAxis}1`], seg[`${fixedAxis}2`]] = [seg[`${fixedAxis}2`], seg[`${fixedAxis}1`]];
      }
    ...
    for (const key in groupedByAxis) {
      ...
      group.sort((a: any, b: any) => a[`${slidingAxis}1`] - b[`${slidingAxis}1`]);
      ...
      for (let i = 1; i < group.length; i++) {
        ...
        if (current[`${slidingAxis}2`] >= next[`${slidingAxis}1`] - EPSILON) {
          current[`${slidingAxis}2`] = Math.max(current[`${slidingAxis}2`], next[`${slidingAxis}2`]);
        } else {
          merged.push({ ...current });
          current = next;
        }
