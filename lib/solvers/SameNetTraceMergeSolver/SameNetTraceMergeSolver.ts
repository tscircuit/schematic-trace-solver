...
import { MERGE_DISTANCE_THRESHOLD } from "../trace-merger-solver"
...
function rangesOverlap(
  ...
  return (
    lo1 <= hi2 + MERGE_DISTANCE_THRESHOLD &&
    lo2 <= hi1 + MERGE_DISTANCE_THRESHOLD
  )
}

function tryMergeTraces(a: TracePath, b: TracePath): TracePath | null {
  const ap = a.points ?? a.tracePath
  const bp = b.points ?? b.tracePath
...
export class SameNetTraceMergeSolver extends BaseSolver {
  ...
  constructor(params: { traces: TracePath[] }) {
    super()
    this.traces = params.traces
  }

  override _step() {
    const result = new SameNetTraceMergeSolver({ traces: this.traces }).solve()
    this.outputTraces = result.traces
    this.solved = true
  }

  override solve(): { traces: TracePath[] } {
    let traces = this.traces.slice()
    let changed = true
