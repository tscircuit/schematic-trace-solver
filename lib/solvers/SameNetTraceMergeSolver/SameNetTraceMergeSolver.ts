...
export class SameNetTraceMergeSolver extends BaseSolver {
   ...
   */
  private _tryMergePaths(pathA: Point[], pathB: Point[]): Point[] | null {
    ...
    for (let ia = 0; ia < segsA.length; ia++) {
      ...
      for (let ib = 0; ib < segsB.length; ib++) {
        ...
        if (isHorizA) {
          ...
          const newX1 = Math.min(Math.min(sa.x1, sa.x2), Math.min(sb.x1, sb.x2))
          const newX2 = Math.max(Math.max(sa.x1, sa.x2), Math.max(sb.x1, sb.x2))
          ...
          const newY1 = Math.min(Math.min(sa.y1, sa.y2), Math.min(sb.y1, sb.y2))
          const newY2 = Math.max(Math.max(sa.y1, sa.y2), Math.max(sb.y1, sb.y2))
  ...
  private _buildMergedPath(
    ...
    const distToNew1 = Math.hypot(
      origStart.x - newStart.x,
      origStart.y - newStart.y,
    )
    const distToNew2 = Math.hypot(
      origStart.x - newEnd.x,
      origStart.y - newEnd.y,
    )
