...
import {
  ...
  railIsOnFacingSide,
} from "./getAlignedParallelJunctionPaths"
  ...
  for (const alignReturnBranches of [false, true]) {
    ...
    for (let donorIndex = 0; donorIndex < donorTraceIds.length; donorIndex++) {
      ...
      for (const branchTrace of outputTraces) {
        ...
        // Align local rails and parallel-pin exits before return branches. If
        // an outer return column meets another net, try shorter escapes.
        let candidatePaths = [
