...
export class CollinearTraceMergeSolver extends BaseSolver {
  ...
  private _mergeCollinearSegments(
    ...
    return traces.map((trace) => {
      ...
      if (newPath.length < 2) {
        ...
        return {
          ...trace,
          tracePath: [original[0]!, original[original.length - 1]!],
        }
      }
  ...
  private _applyHorizontalMerge(
    ...
    for (const mergedInterval of merged) {
      const covered = segs.filter(
        (s) =>
          s.lo >= mergedInterval.lo - EPSILON &&
          s.hi <= mergedInterval.hi + EPSILON,
      )
      ...
      primaryPath[primary.segIdx] = {
        x: leftward ? mergedInterval.lo : mergedInterval.hi,
        y,
      }
      primaryPath[primary.segIdx + 1] = {
        x: leftward ? mergedInterval.hi : mergedInterval.lo,
        y,
      }
  ...
  private _applyVerticalMerge(
    ...
    for (const mergedInterval of merged) {
      ...
      primaryPath[primary.segIdx] = {
        x,
        y: downward ? mergedInterval.lo : mergedInterval.hi,
      }
      primaryPath[primary.segIdx + 1] = {
        x,
        y: downward ? mergedInterval.hi : mergedInterval.lo,
      }
