...
export const mergeSameNetTraces = (
  ...
  const netIds = Array.from(new Set(currentTraces.map(t => t.globalConnNetId)))
      ...
      for (let i = 0; i < netTraces.length; i++) {
        for (let j = i + 1; j < netTraces.length; j++) {
          ...
          if (dist(p1_end, p2_start) < EPS) {
             if (isCollinear(t1.tracePath[t1.tracePath.length-2], p1_end, t2.tracePath[1])) {
               mergedPath = [...t1.tracePath, ...t2.tracePath.slice(1)]
             }
          } else if (dist(p1_end, p2_end) < EPS) {
             if (isCollinear(t1.tracePath[t1.tracePath.length-2], p1_end, t2.tracePath[t2.tracePath.length-2])) {
               mergedPath = [...t1.tracePath, ...t2.tracePath.slice(0, -1).reverse()]
             }
             ...
             if (isCollinear(t1.tracePath[1], p1_start, t2.tracePath[t2.tracePath.length-2])) {
          ...
          if (mergedPath) {
            netTraces[i] = {
              ...
              mspPairId: t1.mspPairId === t2.mspPairId ? t1.mspPairId : `${t1.mspPairId}_${t2.mspPairId}`
            }
