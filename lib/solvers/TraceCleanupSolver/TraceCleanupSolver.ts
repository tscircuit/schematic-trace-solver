...
export class TraceCleanupSolver extends BaseSolver {
  ...
  private _runMergeSameNetTracesStep() {
    ...
    const getTraceNetId = (trace: any) => trace.netId ?? trace.userNetId
    const targetNetId = getTraceNetId(targetTrace)

    const sameNetTraces = this.outputTraces.filter(
      (t: any) => getTraceNetId(t) === targetNetId && t.mspPairId !== targetId,
    )
