  ...
  private _mergeNearbySameNetTraces() {
    ...
    for (let i = 0; i < traces.length; i++) {
      ...
      for (let j = i + 1; j < traces.length; j++) {
        ...
        const sameNet = Object.values(this.input.mergedLabelNetIdMap).some(
          (netSet) =>
        ...
        if (dist(aEnd, bStart) < threshold) {
          base = {
            ...base,
            tracePath: [...base.tracePath, ...other.tracePath],
            ...
            pinIds: Array.from(new Set([...base.pinIds, ...other.pinIds])),
          }
          used.add(j)
        }
      }
