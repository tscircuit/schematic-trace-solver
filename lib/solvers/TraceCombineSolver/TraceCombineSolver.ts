...
export class TraceCombineSolver extends BaseSolver {
  ...
  override _step() {
    ...
    for (const netId in tracesByNet) {
      ...
      for (const trace of traces) {
        trace.mspConnectionPairIds?.forEach(id => allMspConnectionPairIds.add(id))
        ...
        for (let i = 0; i < trace.tracePath.length - 1; i++) {
          ...
          if (Math.abs(p1.y - p2.y) < 0.001) {
            hSegments.push({ x1: Math.min(p1.x, p2.x), x2: Math.max(p1.x, p2.x), y: p1.y })
          } else if (Math.abs(p1.x - p2.x) < 0.001) {
            vSegments.push({ y1: Math.min(p1.y, p2.y), y2: Math.max(p1.y, p2.y), x: p1.x })
          }
        }
      }

      // 2. Snap and Merge Horizontal
      hSegments = this.snapAndMerge(hSegments, "y", "x1", "x2", this.input.distanceThreshold)
      // 3. Snap and Merge Vertical
      vSegments = this.snapAndMerge(vSegments, "x", "y1", "y2", this.input.distanceThreshold)
      ...
      if (hSegments.length > 0) {
        ...
        const avgY = hSegments.reduce((sum, s) => sum + s.y, 0) / hSegments.length
  ...
  private snapAndMerge<T>(
    ...
    const sorted = [...segments].sort((a, b) => (a[coordKey] as any) - (b[coordKey] as any))
    const merged: T[] = []
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if (Math.abs((s[coordKey] as any) - (last[coordKey] as any)) < threshold) {
            currentGroup.push(s)
        } else {
  ...
  private mergeOverlap<T>(group: T[], coordKey: keyof T, startKey: keyof T, endKey: keyof T): T[] {
    if (group.length === 0) return []
    const avgCoord = group.reduce((sum, s) => sum + (s[coordKey] as any), 0) / group.length
    
    // Sort by start
    const sorted = group.map(s => ({...s, [coordKey]: avgCoord}))
    ...
    for (let i = 1; i < sorted.length; i++) {
        ...
        if ((next[startKey] as any) <= (current[endKey] as any)) {
            current[endKey] = Math.max((current[endKey] as any), (next[endKey] as any)) as any
        } else {
            result.push(current)
            current = next
        }
