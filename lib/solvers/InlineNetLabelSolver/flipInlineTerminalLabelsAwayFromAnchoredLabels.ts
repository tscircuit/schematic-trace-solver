...
export const flipInlineTerminalLabelsAwayFromAnchoredLabels = ({
  ...
  for (let index = 0; index < output.length; index++) {
    ...
    const isBlocked =
      anchoredBounds.some((anchor) =>
        boundsOverlap(labelBounds, anchor.bounds),
      ) ||
      traces.some(
        (trace) =>
          trace.globalConnNetId !== label.globalConnNetId &&
          pathIntersectsBounds(trace.tracePath, labelBounds),
      ) ||
      output.some(
        (other, otherIndex) =>
          otherIndex !== index &&
          ...
          other.stubTracePath &&
          pathIntersectsBounds(other.stubTracePath, labelBounds),
      )
    if (!label.stubTracePath || !isBlocked) continue
