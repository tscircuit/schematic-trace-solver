...
export function visualizeSingleNetLabelPlacementSolver(
  ...
  const _groupStroke = getColorFromString(groupId, 0.9)
  const _groupFill = getColorFromString(groupId, 0.5)

  for (const trace of Object.values(solver.inputTraceMap)) {
    const _isHost = host ? trace.mspPairId === host.mspPairId : false
    graphics.lines!.push({
